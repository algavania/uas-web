...
<body class="bg-[#F6F9FE] h-full">
    ...
    <div class="sm:ml-64 h-full">
        <div class="py-8 px-6 bg-[#F6F9FE] h-full w-full relative">
            <div class="flex place-content-end">
                <a type="button" href="lecturer_form.php" class="text-white bg-primary focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-5 focus:outline-none">Add Lecturer</a>
            </div>
            ...
            <div class="w-full relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody id="table-body">
                        <?php
                        include "controller/lecturer/read.php";
                        ?>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        ...
        function search() {
            ...
            xhttp.open("GET", "controller/lecturer/search.php?keyword=" + keyword.value, true);
            xhttp.send();
        }
        ...
        $(".delete").off("click").on("click", async function() {
            var nrp = $(this).attr("data-nip");
            var willDelete = await swal({
                title: "Confirmation",
                text: "Do you want to delete NIP " + nrp + "?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            });
            if (willDelete) {
                $.ajax({
                    url: "controller/lecturer/delete_action.php?nip=" + nrp,
                    type: "POST",
