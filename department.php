...
<body class="bg-[#F6F9FE] h-full">
    ...
    <div class="sm:ml-64 h-full">
        <div class="py-8 px-6 bg-[#F6F9FE] h-full w-full relative">
            <div class="flex place-content-end">
                <button data-modal-target="departmentModal" data-modal-toggle="departmentModal" id="add-button" class="text-white bg-primary focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-5 focus:outline-none cursor-pointer">Add Department</button>
            </div>
            ...
            <div class="w-full relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody id="table-body">
                        <?php
                        include "controller/department/read.php";
                        ?>
                    </tbody>
                </table>
            </div>
        ...
        <div id="departmentModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-96 max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <form action="controller/department/add_action.php" method="post">
                        <div class="flex items-start bg-primary justify-between p-4 border-b rounded-t dark:border-gray-600">
                            <h3 class="text-xl font-semibold text-white">
                                Department Form
                            </h3>
                            <button type="button" class="text-gray-400 bg-transparent hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:text-white" data-modal-hide="departmentModal">
                        ...
                        <div class="p-6 space-y-6">
                            <div>
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 text-black">Department Name</label>
    ...
    <script>
        ...
        function search() {
            ...
            xhttp.open("GET", "controller/department/search.php?keyword=" + keyword.value, true);
            xhttp.send();
        }
        ...
        $("#add-button").off("click").on("click", async function() {
            ...
            $("form").attr('action', 'controller/department/add_action.php');
        });
        ...
        $(".edit").off("click").on("click", async function() {
            ...
            $("#name").val(name);
            $("#id").val(id);
            $("form").attr('action', 'controller/department/update_action.php');
        });
        ...
        $(".delete").off("click").on("click", async function() {
            ...
            if (willDelete) {
                $.ajax({
                    url: "controller/department/delete_action.php?id=" + id,
                    type: "POST",
