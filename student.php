...
<body class="bg-[#F6F9FE] h-full">
    ...
    <div class="sm:ml-64 h-full">
        <div class="py-8 px-6 bg-[#F6F9FE] h-full w-full relative">
            <div class="flex place-content-end">
                <a type="button" href="student_form.php" class="text-white bg-primary focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-5 focus:outline-none">Add Student</a>
            </div>
            ...
            <div class="w-full relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody id="table-body">
                        <?php
                        include "controller/mahasiswa/read.php";
                        ?>
                    </tbody>
                </table>
            </div>
    ...
    <script>
        ...
        function search() {
            ...
            xhttp.open("GET", "controller/mahasiswa/search.php?keyword=" + keyword.value, true);
            xhttp.send();
        }
        ...
        $(".delete").off("click").on("click", async function() {
            ...
            var willDelete = await swal({
                title: "Confirmation",
                text: "Do you want to delete NRP " + nrp + "?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            });
            if (willDelete) {
                $.ajax({
                    url: "controller/mahasiswa/delete_action.php?nrp=" + nrp,
                    type: "POST",
