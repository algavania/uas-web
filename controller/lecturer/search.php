...
$sql = "SELECT *, users.name AS user_name, departments.name AS department_name FROM lecturers LEFT JOIN departments ON departments.id=lecturers.departments LEFT JOIN users ON users.id=lecturers.user_id WHERE nip LIKE '%$keyword%' OR users.name LIKE '%$keyword%' OR departments.name LIKE '%$keyword%'";
